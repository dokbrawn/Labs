...
fn run_backend(pg_conn: &str, args: &[&str]) -> Result<String, String> {
    ...
    let out = Command::new(&bin)
        ...
        .env("LIBRARY_ENABLE_NET", "1")  // Включаем API
        .env("OFFLINE_MODE", "0")         // Не офлайн режим
        .current_dir(parent)
        .output()
    ...
    // Игнорируем PostgreSQL NOTICE сообщения
    let is_critical = out.status.code() != Some(0) 
        && !stderr.contains("ЗАМЕЧАНИЕ") 
        && !stderr.contains("NOTICE")
        && !stderr.contains("уже существует");
...
impl LibraryGui {
    ...
    fn add_book(&mut self) {
        ...
        // --fetch-network включает подгрузку из OpenLibrary API
        let result = run_backend(&self.pg_conn, &["upsert", &file_str, "--fetch-network"]);
        let _ = fs::remove_file(file);
        
        match result {
            Ok(_) => {
                self.status = "Книга добавлена (с подгрузкой из OpenLibrary)".to_string();
...
impl App for LibraryGui {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("header").show(ctx, |ui| {
            ...
            ui.label("PostgreSQL + иерархия жанров + OpenLibrary API");
            
            ui.horizontal(|ui| {
                ui.label("LIBRARY_PG_CONN:");
                ui.text_edit_singleline(&mut self.pg_conn);
                if ui.button("Init").clicked() { self.init_backend(); }
                if ui.button("Refresh").clicked() { self.refresh_books(); }
            });
        ...
        egui::SidePanel::left("left_filters")
            ...
            .show(ctx, |ui| {
                ...
                ui.separator();
                ui.label("Поиск (title → author)");
                let search = ui.text_edit_singleline(&mut self.query);
                if search.changed() { self.apply_filters(); }
                ...
                egui::ComboBox::from_id_salt("sort_field")
                    ...
                    .show_ui(ui, |ui| {
                        for f in [SortField::Title, SortField::Author, SortField::Year, 
                                  SortField::Rating, SortField::Price] {
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                while i < self.shown_books.len() {
                    ui.horizontal_wrapped(|ui| {
                        for _ in 0..cols {
                            if i >= self.shown_books.len() { break; }
